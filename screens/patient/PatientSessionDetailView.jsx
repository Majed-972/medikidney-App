...
const PatientSessionDetailView = ({ route, navigation }) => {
  ...
  const hasValidParams = Boolean(sessionId && patientId);

  const invalidParamsView = (
  ...
  const fetchSession = useCallback(async () => {
    ...
    console.log(
      '[PatientSessionDetailView] fetchSession started with sessionId:',
      sessionId,
      'patientId:',
      patientId
    );
    try {
      ...
      try {
        ...
        console.log(
          '[PatientSessionDetailView] last-session failed or mismatched, trying individual endpoints:',
          e.message
        );
      }

      const [basicRes, vitalsRes, settingsRes, symptomsRes, medicationsRes] =
        await Promise.allSettled([
          ...
          api
            .get(`/dialysis-sessions/${sessionId}/details/medications`)
            .catch(() => ({ data: [] })),
        ]);
      ...
      if (basicRes.status === 'fulfilled') {
        ...
        console.log(
          '[PatientSessionDetailView] basicRes data:',
          JSON.stringify(s).substring(0, 200)
        );
        sessionData = s;
        ...
        console.log(
          '[PatientSessionDetailView] individual endpoint failed (status:',
          errorStatus,
          '), trying sessions list fallback...'
        );
      ...
      if (settingsRes.status === 'fulfilled') {
        ...
        if (Array.isArray(obj)) {
          ...
          obj &&
          typeof obj === 'object' &&
          (obj.blood_flow_rate != null || obj.bloodFlowRate != null)
        ) {
  ...
  }, [hasValidParams, patientId, sessionId, t.patientSessionDetail.fetchError]);
  ...
  const formatTime = (timeStr) => {
    ...
    return dateObj.toLocaleTimeString(t.cancel === 'إلغاء' ? 'ar-EG' : 'en-US', {
      hour: '2-digit',
      minute: '2-digit',
    });
  };
  ...
  const { session, patient, nurse, schedule, vitalSigns, medications, dialysisSettings, symptoms } =
    data;
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        <View style={styles.card}>
          <SectionHeader
            ...
            title={t.patientSessionDetail.patientNurseInfo}
            color="#193B6B"
          />
          ...
          {patient?.allergies && (
            <View style={styles.allergyRow}>
              ...
              <Text style={styles.allergyText}>
                {t.patientSessionDetail.allergy} {patient.allergies}
              </Text>
            </View>
          )}
        </View>
        ...
        <View style={styles.card}>
          <SectionHeader
            icon="calendar-clock"
            title={t.patientSessionDetail.sessionInfo}
            color="#26CDD6"
          />
          ...
          <View style={styles.twoCol}>
            <InfoRow
              label={t.patientSessionDetail.startTime}
              value={formatTime(session?.start_time)}
            />
            <InfoRow
              label={t.patientSessionDetail.endTime}
              value={formatTime(
                session?.end_time || (session?.status === 'COMPLETED' ? session?.updated_at : null)
              )}
            />
          </View>
          <Divider style={styles.rowDivider} />
          {schedule && (
            <>
              <View style={styles.twoCol}>
                <InfoRow
                  label={t.patientSessionDetail.day}
                  value={t.days[schedule.weekday] || schedule.weekday}
                />
                <InfoRow
                  label={t.patientSessionDetail.shiftNumber}
                  value={`${t.patientSessionDetail.shiftPrefix} ${schedule.shift_number}`}
                />
              </View>
              <Divider style={styles.rowDivider} />
              <InfoRow
                label={t.patientSessionDetail.machineNumber}
                value={`${t.patientSessionDetail.machinePrefix}${schedule.machine_number}`}
              />
              <Divider style={styles.rowDivider} />
            </>
          )}
          ...
          {(session?.weight_before != null || session?.weight_after != null) && (
            <>
              ...
              {(() => {
                ...
                const ufRaw = lastSetting
                  ? parseFloat(lastSetting.ultrafiltration_rate ?? lastSetting.ultrafiltrationRate)
                  : null;
                ...
                return (
                  <View style={styles.fluidRemovedRow}>
                    ...
                    <Text style={styles.fluidRemovedText}>
                      ...
                      <Text style={styles.fluidRemovedValue}>{ufLiters} لتر</Text>
                    </Text>
                  </View>
                );
              })()}
        ...
        <View style={styles.card}>
          <SectionHeader
            icon="heart-pulse"
            title={t.patientSessionDetail.vitalSigns}
            color="#DE1A1C"
          />
            ...
            vitalSigns.map((v, idx) => (
              <View key={v.vital_id || idx}>
                ...
                <View style={styles.vitalsGrid}>
                  <MetricChip
                    label={t.patientSessionDetail.systolic}
                    value={v.systolic ? `${v.systolic}` : '—'}
                    icon="heart-pulse"
                    color="#DE1A1C"
                  />
                  <MetricChip
                    label={t.patientSessionDetail.diastolic}
                    value={v.diastolic ? `${v.diastolic}` : '—'}
                    icon="heart-outline"
                    color="#f97316"
                  />
                  <MetricChip
                    label={t.patientSessionDetail.pulse}
                    value={v.pulse ? `${v.pulse} bpm` : '—'}
                    icon="pulse"
                    color="#8b5cf6"
                  />
                  <MetricChip
                    label={t.patientSessionDetail.temperature}
                    value={v.temperature ? `${v.temperature}°C` : '—'}
                    icon="thermometer"
                    color="#f59e0b"
                  />
                  {v.oxygen_saturation != null && (
                    <MetricChip
                      label={t.patientSessionDetail.oxygen}
        ...
        <View style={styles.card}>
          <SectionHeader
            icon="cog-outline"
            title={t.patientSessionDetail.machineSettings}
            color="#26CDD6"
          />
          {dialysisSettings && dialysisSettings.length > 0 ? (
            dialysisSettings.map((s, idx) => (
              <View key={s.setting_id || idx}>
                ...
                <View style={styles.vitalsGrid}>
                  <MetricChip
                    label={t.patientSessionDetail.bloodFlow}
                    value={`${s.blood_flow_rate} ml/min`}
                    icon="water-pump"
                    color="#26CDD6"
                  />
                  <MetricChip
                    label={t.patientSessionDetail.dialysateFlow}
                    value={`${s.dialysate_flow} ml/h`}
                    icon="beaker-outline"
                    color="#193B6B"
                  />
                  <MetricChip
        ...
        <View style={styles.card}>
          <View style={styles.sectionHeaderRow}>
            <SectionHeader
              ...
              title={t.patientSessionDetail.symptoms}
              color="#f59e0b"
            />
            {symptoms?.total > 0 && (
                  ...
                  {Object.entries(symptoms.breakdown).map(([type, severities]) => {
                    ...
                    return (
                      <View key={type} style={styles.breakdownChip}>
                        <Text style={styles.breakdownType}>{t.symptomsDict[type] || type}</Text>
