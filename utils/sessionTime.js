...
const TIME_ONLY_ISO_REGEX = /^1970-01-01T([01]\d|2[0-3]):([0-5]\d):([0-5]\d)(?:\.(\d{1,3}))?Z$/;
...
export const formatSessionTime = (value, { locale = 'ar-EG', fallback = '--:--' } = {}) => {
  if (!value) return fallback;

  const timeValue = String(value).trim();
  const timeOnlyMatch = TIME_ONLY_REGEX.exec(timeValue);
  if (timeOnlyMatch) {
    const [, hoursText, minutesText, secondsText, millisecondsText] = timeOnlyMatch;
