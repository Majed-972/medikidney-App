...
const _validateWeight = (val, fieldName, required = true) => {
  if (!val || val.trim() === '') {
...
const getBpStatus = (sys, dia, t) => {
  if (sys > 140 || dia > 90)
    return { label: t.vitalSigns.status.high, color: '#DE1A1C', bg: '#FBEAEA' };
  if (sys < 90 || dia < 60)
    return { label: t.vitalSigns.status.low, color: '#A32D2F', bg: '#FBEAEA' };
      ...
      <View style={cardStyles.header}>
        ...
        <View style={cardStyles.headerLeft}>
          <View style={[cardStyles.statusBadge, { backgroundColor: bpStatus.bg }]}>
            <Text style={[cardStyles.statusText, { color: bpStatus.color }]}>{bpStatus.label}</Text>
          </View>
          <Pressable onPress={() => onDelete(vitalId)} style={cardStyles.deleteBtn}>
      ...
      <View style={cardStyles.values}>
        ...
        {item.oxygen_saturation != null && (
          <>
            <View style={cardStyles.divider} />
            <View style={cardStyles.valueBox}>
              <Text style={[cardStyles.bigNum, { color: '#26CDD6' }]}>
...
const VitalSignsTab = ({ route }) => {
  ...
  const fetchVitals = async () => {
    if (!sessionId) return;
    try {
      const { data } = await api.get(`/dialysis-sessions/${sessionId}/details/vital-signs`);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await api.post(`/dialysis-sessions/${sessionId}/details/vital-signs`, {
  ...
  const handleDelete = async (vitalId) => {
        ...
        : await new Promise((resolve) => {
            Alert.alert(
              ...
              [
                ...
                {
                  text: t.vitalSigns.deleteConfirmTitle.replace('تأكيد ', ''),
    ...
    try {
      await api.delete(`/dialysis-sessions/${sessionId}/details/vital-signs/${numericId}`);
      setVitals((prev) => prev.filter((v) => (v.vital_id || v.id) !== numericId));
    } catch (err) {
      console.log('Delete vital err:', err.response?.data || err.message);
      Platform.OS === 'web'
        ? alert(t.vitalSigns.deleteFailed)
        : Alert.alert(t.error, t.vitalSigns.deleteFailed);
    }
  };
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      <View style={styles.topBar}>
        ...
        >
          <MaterialCommunityIcons name={showForm ? 'close' : 'plus'} size={18} color="#fff" />
          <Text style={styles.addBtnText}>{showForm ? t.cancel : t.vitalSigns.newReading}</Text>
        </Pressable>
          ...
          <View style={styles.latestRow}>
            ...
            {vitals[0].oxygen_saturation != null && (
              <>
                <View style={styles.latestDivider} />
                <View style={styles.latestItem}>
                  <Text style={[styles.latestVal, { color: '#BCEFF3' }]}>
