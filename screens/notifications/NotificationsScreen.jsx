...
const hasWeightAfter = (session) => session?.weight_after != null || session?.weightAfter != null;
...
const NotificationsScreen = () => {
      ...
      list.map(async (notification) => {
        ...
        try {
          ...
          if (hasWeightAfter(session)) {
            await api
              .patch(`/notifications/${notification.notification_id}/delete`)
              .catch(() => {});
            return null;
          }
        } catch (error) {
          console.log(
            'Could not verify weight notification:',
  ...
  const handleNotificationTap = async (notification) => {
    ...
    try {
      ...
      // Ignore devices/environments where haptics are unavailable.
    }
    ...
    switch (notificationType) {
      ...
      case 'SESSION_NO_WEIGHT':
        ...
        try {
          ...
          console.log(
            'Could not verify session weight before navigation:',
            error.response?.data || error.message
          );
        }
  ...
  const renderNotification = ({ item }) => (
    <TouchableOpacity activeOpacity={0.85} onPress={() => handleNotificationTap(item)}>
            ...
            <View style={styles.titleContainer}>
              ...
              <Text style={styles.date}>
                {new Date(item.created_at).toLocaleDateString(
                  t.vitalSigns.now === 'الآن' ? 'ar-SA' : 'en-US',
                  {
                    year: 'numeric',
                    month: 'long',
                    day: 'numeric',
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        <TouchableOpacity onPress={() => navigation.goBack()} style={styles.backButton}>
          <MaterialIcons
            name={t.vitalSigns.now === 'الآن' ? 'chevron-right' : 'chevron-left'}
            size={28}
            color="#193B6B"
          />
        </TouchableOpacity>
        <View>
          ...
          {unreadCount > 0 && (
            <Text style={styles.unreadText}>
              {unreadCount}{' '}
              {unreadCount === 1 ? t.notifications.newSingular : t.notifications.newPlural}
            </Text>
          )}
        </View>
