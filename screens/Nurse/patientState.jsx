...
import { buildLocalSessionDatePayload, buildLocalTimePayload } from '../../utils/sessionTime';
...
const PatientState = ({ route }) => {
      ...
      [
        { text: 'تراجع', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              setLoading(true);
              await api.delete(`/dialysis-scheduling/assign-nurse/${patient.scheduleId}`);
  ...
  const handleConfirmWeight = async () => {
    ...
    try {
      ...
      const createRes = await api.post('/dialysis-sessions', {
      ...
      if (updatedPatient && updatedPatient.sessionId) {
        ...
        const fallbackId =
          createRes.data?.session_id || createRes.data?.sessionId || createRes.data?.id;
      ...
      if (typeof msg === 'string' && (msg.includes('موجود') || msg.includes('exist'))) {
        ...
        setWeightInputError(Array.isArray(msg) ? msg.join(' ') : (msg ?? 'فشل بدء الجلسة'));
      }
    } finally {
  ...
  const getStatusInfo = (status) => {
    switch (status) {
      case 'COMPLETED':
        return { label: 'مكتملة', color: '#26CDD6', icon: 'check-circle', bg: '#E9FAFB' };
      case 'PENDING':
      case 'IN_PROGRESS':
        return { label: 'تم البدء (جاري الغسيل)', color: '#26CDD6', icon: 'sync', bg: '#E9FAFB' };
      case 'CANCELLED':
        return { label: 'ملغية', color: '#DE1A1C', icon: 'close-circle', bg: '#FBEAEA' };
      case 'MISSED':
        return { label: 'غائب', color: '#8296B1', icon: 'account-off', bg: '#f9fafb' };
      default:
        return {
          label: 'لم يتم البدء',
          color: '#8296B1',
  ...
  return (
    <View style={styles.container}>
      ...
      >
        <View style={mStyles.overlay}>
          <View style={mStyles.sheet}>
            ...
            {weightInputError ? <Text style={mStyles.errText}>{weightInputError}</Text> : null}
      ...
      <View style={styles.header}>
        ...
        <Pressable onPress={() => navigation.navigate('SelectPatient')} style={styles.editBtn}>
          ...
          myPatients.map((patient) => {
            ...
            return (
              <View
                ...
                style={[styles.card, { borderRightColor: statusInfo.color }]}
              >
