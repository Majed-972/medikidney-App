...
const SYMPTOMS_LIST = (t) => [
  ...
  {
    id: 'DIZZINESS',
    label: t.symptoms.list.DIZZINESS,
    icon: 'rotate-3d-variant',
    color: '#8296B1',
  },
  {
    id: 'ITCHING',
    label: t.symptoms.list.ITCHING,
    icon: 'hand-back-right-outline',
    color: '#26CDD6',
  },
  {
    id: 'MUSCLE_PAIN',
    label: t.symptoms.list.MUSCLE_PAIN,
    icon: 'arm-flex-outline',
    color: '#A32D2F',
  },
  {
    id: 'OTHER',
    label: t.symptoms.list.OTHER,
...
const SymptomsTab = ({ route }) => {
  ...
  const fetchData = async () => {
    try {
      ...
      const sympRes = await api.get(`/dialysis-sessions/${sessionId}/details/symptoms`);
  ...
  const handleDeleteSymptom = async (id) => {
    try {
      await api.delete(`/dialysis-sessions/${sessionId}/details/symptoms/${id}`);
      fetchData();
    } catch (err) {
  ...
  if (loading && history.length === 0) {
    return (
      <View style={styles.centered}>
        <ActivityIndicator color="#26CDD6" />
      </View>
    );
  }
        ...
        {symptomsList.map((item) => {
          ...
          return (
            ...
            >
              ...
              />
              <Text style={[styles.chipText, isSelected && { color: '#fff' }]}>{item.label}</Text>
            </Pressable>
          );
        })}
      </View>
      ...
      {history.length > 0 && (
        <View style={styles.historySection}>
          <Text style={styles.historyTitle}>
            {t.symptoms.historyTitle} ({history.length})
          </Text>
          {history.map((h, i) => {
