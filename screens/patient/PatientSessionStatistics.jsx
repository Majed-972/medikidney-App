...
const StatCard = ({ title, icon, iconColor, values, unit, goodWhenDown = true, formatVal, t }) => {
  ...
  const avg = validValues.length
    ? (validValues.map(Number).reduce((a, b) => a + b, 0) / validValues.length).toFixed(1)
    : '—';
        ...
        <View style={styles.sparklineContainer}>
          {validValues.slice(-8).map((v, i) => {
            ...
            return (
              <View
                key={i}
                style={[styles.sparkBar, { height: heightPct, backgroundColor: iconColor + '99' }]}
              />
            );
          })}
        </View>
    ...
    <View style={styles.statCard}>
      <View style={styles.statCardHeader}>
        ...
        <View style={[styles.trendBadge, { backgroundColor: '#fef3c7' }]}>
          <Text style={[styles.trendBadgeText, { color: '#92400e' }]}>
            {total} {t.patientSessionStats.symptomCount}
          </Text>
        </View>
      </View>
      <View style={styles.statCardBody}>
...
const PatientSessionStatistics = ({ route, navigation }) => {
  ...
  const fetchStatistics = useCallback(async () => {
    try {
      ...
      try {
        const lastSessionRes = await api.get(
          `/dialysis-sessions/patient/${patientId}/last-session`
        );
  ...
  const formatDate = (d) => {
    if (!d) return '—';
    return new Date(d).toLocaleDateString(t.vitalSigns.now === 'الآن' ? 'ar-EG' : 'en-US', {
      day: 'numeric',
      month: 'short',
    });
  };
  ...
  const fluidRemovedValues = sessions
    .map((s) => s.fluid_removed)
    ...
    <View style={styles.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        <View style={styles.nutritionTip}>
          <Text style={styles.nutritionTipText}>{t.patientSessionStats.nutritionTip}</Text>
        </View>
              ...
              {s.blood_pressure_before && (
                <Text style={styles.sessionRowMetric}>
                  <Text style={{ color: '#64748b', fontSize: 11 }}>
                    {t.patientSessionStats.pressure}{' '}
                  </Text>
                  {s.blood_pressure_before}
                </Text>
              )}
