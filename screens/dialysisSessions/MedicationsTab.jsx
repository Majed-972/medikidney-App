...
const confirmAction = async (msg, t) => {
  ...
  return new Promise((resolve) => {
    Alert.alert(
      t.medications.deleteConfirm.split('?')[0] + '?',
      msg,
      [
...
const MedicationsTab = ({ route }) => {
  ...
  const fetchMeds = async () => {
    ...
    try {
      setLoading(true);
      const { data } = await api.get(`/dialysis-sessions/${sessionId}/details/medications`);
  ...
  const postMed = async (medData) => {
    try {
      ...
      await api.post(`/dialysis-sessions/${sessionId}/details/medications`, {
  ...
  const deleteMed = async (med) => {
    ...
    try {
      ...
      await api.delete(`/dialysis-sessions/${sessionId}/details/medications/${numId}`);
      await fetchMeds();
    } catch (err) {
      ...
      <View style={styles.topBar}>
        ...
        >
          ...
          <Text style={styles.customBtnText}>
            {showCustom ? t.cancel : t.medications.customMed}
          </Text>
        </Pressable>
            ...
            {PRESET_MEDS.map((med) => {
              ...
              return (
                ...
                >
                  ...
                  <Text style={styles.presetDose}>
                    {med.dosage} {med.unit}
                  </Text>
                  {isActive ? (
                    <View style={[styles.presetChip, { backgroundColor: med.color }]}>
                      <Text style={[styles.presetChipText, { color: '#fff' }]}>
                        {t.medications.selected}
                      </Text>
                    </View>
                  ) : (
                    <View style={[styles.presetChip, { backgroundColor: med.color + '15' }]}>
                      <Text style={[styles.presetChipText, { color: med.color }]}>
                        {t.medications.select}
                      </Text>
                    </View>
                  )}
                </Pressable>
            ...
            <View style={[styles.formCard, { borderTopColor: selectedPreset.color }]}>
              ...
              <View style={styles.unitPicker}>
                {UNITS.map((u) => (
                  ...
                  >
                    <Text style={[styles.unitChipText, presetUnit === u && { color: '#fff' }]}>
                      {u}
                    </Text>
                  </Pressable>
                ))}
              ...
              <View
                style={[styles.inputBox, { height: 60, alignItems: 'flex-start', paddingTop: 10 }]}
              >
                <TextInput
              ...
              >
                {isSubmitting ? (
                  ...
                  <Text style={styles.saveBtnText}>
                    {t.medications.register} {selectedPreset.label}
                  </Text>
                )}
              </Pressable>
            </View>
        ...
        <View style={styles.formCard}>
          ...
          <View style={styles.unitPicker}>
            {UNITS.map((u) => (
              ...
              >
                <Text style={[styles.unitChipText, customForm.unit === u && { color: '#fff' }]}>
                  {u}
                </Text>
              </Pressable>
            ))}
