...
const PatientSessionScreen = ({ route, navigation }) => {
    ...
    const startTimestamp =
      startTime && !Number.isNaN(new Date(startTime).getTime())
        ...
        : sessionData?.created_at
          ...
          : null;
    const base = startTimestamp ? Math.floor((Date.now() - startTimestamp) / 1000) : 0;
  ...
  useEffect(() => {
    ...
    reminderRef.current = setTimeout(() => {
      if (phase === 'IN_SESSION') {
        ...
        Alert.alert(t.patientSessionScreen.alertTitle, t.patientSessionScreen.alertMessage, [
  ...
  const handleSaveWeight = async () => {
    ...
    try {
      ...
      setPhase('DONE');
      Alert.alert(
        t.patientSessionScreen.saveSuccessTitle,
        t.patientSessionScreen.saveSuccessMessage
      );
    } catch (err) {
    ...
    return (
      <View style={styles.lockScreen}>
        ...
        <View style={styles.lockHeader}>
          ...
          <Text style={styles.lockSub}>{t.patientSessionScreen.lockSub}</Text>
        </View>

        {weightBefore != null && (
          <View style={styles.lockWeightBefore}>
            <Text style={styles.lockWeightBeforeLabel}>
              {t.patientSessionScreen.weightBeforeLabel}
            </Text>
        ...
        <View style={styles.lockInputCard}>
          ...
          {weightError && <Text style={styles.lockErrText}>{weightError}</Text>}
    ...
    <View style={styles.sessionScreen}>
      ...
      <View style={styles.sessionHeader}>
        <Text style={styles.sessionPatient}>
          {patientName || t.patientSessionScreen.sessionTitle}
        </Text>
      ...
      {weightBefore != null && (
        <View style={styles.weightBeforeCard}>
          ...
          <Text style={styles.weightBeforeText}>
            {t.patientSessionScreen.weightBeforeLabel}:{' '}
            <Text style={{ fontWeight: '800', color: '#26CDD6' }}>{weightBefore} kg</Text>
          </Text>
        </View>
      )}
      ...
      <View style={styles.infoCard}>
        ...
        <Text style={styles.infoText}>{t.patientSessionScreen.infoText}</Text>
      </View>

      <View style={styles.sessionFooter}>
        <Pressable
          style={[styles.endBtn, { backgroundColor: '#193B6B' }]}
