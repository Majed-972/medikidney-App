...
const SessionDetails = ({ route, navigation }) => {
  ...
  const steps = [
    {
      id: 1,
      title: t.sessionDetails.steps.settings,
      component: <SettingsTab route={{ params: { sessionId } }} />,
    },
    {
      id: 2,
      title: t.sessionDetails.steps.vitals,
      component: <VitalSignsTab route={{ params: { sessionId } }} />,
    },
    {
      id: 3,
      title: t.sessionDetails.steps.medications,
      component: <MedicationsTab route={{ params: { sessionId } }} />,
    },
    {
      id: 4,
      title: t.sessionDetails.steps.symptoms,
      component: <SymptomsTab route={{ params: { sessionId } }} />,
    },
  ];
  ...
  const handleFinishWithWeight = async () => {
    ...
    try {
      setIsFinishing(true);
      await api.patch(`/dialysis-sessions/${sessionId}/status`, {
        status: 'COMPLETED',
        weightAfter: num,
  ...
  const handleFinishSkipWeight = async () => {
    try {
      ...
      await SessionTimer.clearTimer(sessionId);
      setEndModalVisible(false);
      Alert.alert(t.sessionDetails.endSession, t.sessionDetails.sessionEndSkipSuccess);
      navigation.goBack();
    } catch (error) {
      console.log('Error finishing session:', error.response?.data || error.message);
  ...
  return (
    <View style={styles.container}>
          ...
          <View style={modalStyles.sheet}>
                ...
                >
                  ...
                  <View style={modalStyles.optionTextBox}>
                    <Text style={modalStyles.optionTitle}>
                      {t.sessionDetails.enterWeightOption}
                    </Text>
                  ...
                  onPress={() => {
                    Alert.alert(
                      ...
                      [
                        ...
                        {
                          text: t.sessionDetails.skipAndEnd,
                          onPress: handleFinishSkipWeight,
                          style: 'destructive',
                        },
                      ]
                    );
            ...
            {endModalPhase === 'weight_input' && (
              <>
                ...
                <View
                  style={[modalStyles.inputRow, weightInputError ? modalStyles.inputErr : null]}
                >
      ...
      <View style={styles.header}>
        ...
        >
          <Pressable
            ...
            style={{ padding: 5, backgroundColor: 'rgba(255,255,255,0.2)', borderRadius: 10 }}
          >
            ...
            onPress={async () => {
              ...
              try {
                const searchRes = await api.get(
                  `/users/profile/patients/search?name=${encodeURIComponent(name)}`
                );

                const matchedPatient =
                  searchRes.data?.find((p) => p.name === name) || searchRes.data?.[0];
      ...
      <View style={styles.stepBar}>
        ...
        <Text style={styles.stepTitleBelow}>
          {t.sessionDetails.step} {step}: {currentStepData.title}
        </Text>
      </View>
