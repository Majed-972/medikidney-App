...
const DoctorCard = ({ doctor, selected, onPress, t }) => (
  <Pressable onPress={onPress} style={[dc.card, selected && dc.cardSelected]}>
    <View style={[dc.avatar, selected && dc.avatarSelected]}>
      <MaterialCommunityIcons name="doctor" size={28} color={selected ? C.white : C.teal} />
    </View>
    <View style={{ flex: 1 }}>
    ...
    {selected && <MaterialCommunityIcons name="check-circle" size={22} color={C.white} />}
  </Pressable>
);
...
const TimeSlot = ({ slot, selected, onPress, formatTime, t }) => (
  ...
  >
    ...
    {slot.booked && <Text style={ts.bookedTag}>{t.appointments.booked}</Text>}
  </Pressable>
);
...
const AppointmentScreen = () => {
  ...
  const [, setSlotBookingAllowed] = useState(true);
  ...
  const handleDaySelect = async (day) => {
    ...
    try {
      ...
      if (
        isToday(date) &&
        filtered.filter((s) => !s.booked).length === 0 &&
        all.filter((s) => !s.booked).length > 0
      ) {
        setSlots([]);
  ...
  const handleCancelAppointment = (apptId) => {
    ...
    Alert.alert(t.appointments.cancelTitle, t.appointments.cancelMsg, [
    ...
    try {
      ...
      const isSameDay =
        ...
        (msg.includes('already') ||
          ...
          msg.includes('مسبق'));
      Alert.alert(
        isSameDay ? t.appointments.sorry : t.failed,
        isSameDay ? t.appointments.sameDayError : msg || t.appointments.bookFailed
      );
    } finally {
    ...
    <View>
        ...
        <Pressable onPress={() => goToStep(1)} style={s.summaryPill}>
          ...
          <Text style={s.summaryPillText}>
            {(() => {
              ...
              d.setDate(d.getDate() + diff);
              const dayName =
                t?.days?.[selectedDay?.weekday] || DAY_AR2[selectedDay?.weekday] || '';
              return `${dayName} ${d.getDate()} ${MONTH_AR2[d.getMonth()]}`;
            })()}
          </Text>
        </Pressable>
        <Pressable onPress={() => goToStep(0)} style={[s.summaryPill, { backgroundColor: C.gray }]}>
          <Text style={[s.summaryPillText, { color: C.muted }]}>
            د. {selectedDoctor?.full_name}
          </Text>
        </Pressable>
      </View>
      ...
      {errorMsg ? (
        ...
        <View style={s.emptyBox}>
          <MaterialCommunityIcons name="calendar-remove-outline" size={40} color={C.muted} />
          <Text style={s.emptyText}>لا توجد مواعيد متاحة</Text>
        </View>
      ) : (
              ...
              <View key={appt.appointment_id} style={s.apptCard}>
                ...
                <View style={{ flex: 1, alignItems: 'flex-end' }}>
                  ...
                  <Text style={s.apptDate}>
                    {appt.appt_date} — {formatTime(appt.appt_time)}
                  </Text>
