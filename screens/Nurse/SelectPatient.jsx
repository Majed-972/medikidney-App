...
const SelectPatient = () => {
  ...
  const togglePatient = (patient) => {
    if (patient.assignedNurseId !== null) {
      const msg =
        patient.assignedNurseId === myNurseId
  ...
  const handleProceed = async () => {
    ...
    try {
      ...
      for (const patientId of newOnly) {
        ...
        if (patient) {
          try {
            await api.post('/dialysis-scheduling/assign-nurse', { scheduleId: patient.scheduleId });
          } catch (err) {
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        <View style={styles.headerTop}>
          ...
          <Pressable
            onPress={goToStatus}
            style={[styles.stateBtn, confirmedIds.length === 0 && styles.stateBtnDisabled]}
          >
            ...
            >
              {t.selectPatient.dialysisStatus}{' '}
              {confirmedIds.length > 0 ? `(${confirmedIds.length})` : ''}
            </Text>
          </Pressable>
        </View>
        ...
        <ScrollView
          ...
          contentContainerStyle={styles.shiftTabs}
        >
          {shifts.map((s) => (
            <Pressable
              key={s.shiftNumber}
            ...
            >
              ...
              <View
                style={[
                  styles.countBadge,
                  activeShift === s.shiftNumber && styles.activeCountBadge,
                ]}
              >
        ...
        {loading ? (
          ...
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptyText}>
              {searchQuery
                ? (t.selectPatient.noSearchResults ?? 'لا توجد نتائج للبحث')
                : t.selectPatient.noPatients}
            </Text>
          </View>
        ) : (
          filteredPatients.map((patient) => {
            ...
            const isTakenByOther =
              patient.assignedNurseId !== null && patient.assignedNurseId !== myNurseId;
              ...
              >
                ...
                <View style={styles.info}>
                  ...
                  <Text style={styles.sub}>
                    {isDone
                      ? t.selectPatient.sessionComplete
                      : isConfirmed
                        ? t.selectPatient.confirmedInList
                        : isTakenByOther
                          ? `${t.selectPatient.withNurse} ${patient.assignedNurseName}`
