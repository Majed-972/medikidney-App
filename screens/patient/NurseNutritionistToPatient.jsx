...
const StaffPatientView = ({ route, navigation }) => {
  ...
  const fetchMedicalTests = async (id) => {
    try {
      ...
      console.log('Error tests:', e.message);
    }
  };
  ...
  const fetchRadiology = async (id) => {
    try {
      ...
      console.log('Error rad:', e.message);
    }
  };
  ...
  const fetchPatientData = useCallback(async () => {
    try {
      ...
      const storedRole =
        (await AsyncStorage.getItem('userRole')) || (await AsyncStorage.getItem('role'));
      ...
      if (storedUser) {
        try {
          ...
          finalRole =
            parsed.role || parsed.user?.role || parsed.data?.user?.role || storedRole || 'GUEST';
        } catch (e) {
      ...
      const response = await api.get(`/users/profile/patients/${patientId}`);
  ...
  const handleDownload = async (id, type) => {
    if (!id) {
      Alert.alert(
        t.patientSessionScreen.alertTitle || 'تنبيه',
        t.staffPatientView.fileNotAvailable
      );
      return;
    }
    try {
      ...
      if (!fullUrl) {
        Alert.alert(t.patientProfile.errorTitle || 'خطأ', t.staffPatientView.fileLinkError);
        return;
      }
    ...
    <View style={styles.reportCard}>
      ...
      <View style={styles.reportContent}>
        <Text style={styles.reportDetail}>
          <Text style={styles.boldLabel}>{t.staffPatientView.doctor}</Text> د.{' '}
          {doctor || t.staffPatientView.notSpecified}
        </Text>
        <Text style={styles.reportDetail}>
          <Text style={styles.boldLabel}>{t.staffPatientView.description}</Text>{' '}
          {description || t.staffPatientView.noDesc}
        </Text>
        <View
          style={[
            styles.statusBadge,
            {
              backgroundColor: status === 'PENDING' || status === 'pending' ? '#FBEAEA' : '#E9FAFB',
            },
          ]}
        >
          <Text
            style={[
              styles.statusText,
              { color: status === 'PENDING' || status === 'pending' ? '#A32D2F' : '#193B6B' },
            ]}
          >
        ...
        <View style={styles.patientHeaderRow}>
          ...
          <View style={styles.patientHeaderInfo}>
            <Text style={styles.patientNameText}>
              {patient?.full_name || t.staffPatientView.patientNameFallback}
            </Text>
          </View>
        </View>
      </View>
        ...
        <Tab.Item
          ...
          icon={
            <Icon
              ...
              type="material-community"
              size={18}
              color={tabIndex === 0 ? '#193B6B' : '#8296B1'}
            />
          }
        />
        <Tab.Item
          ...
          icon={
            <Icon
              ...
              type="material-community"
              size={18}
              color={tabIndex === 1 ? '#193B6B' : '#8296B1'}
            />
          }
        />
        <Tab.Item
          ...
          icon={
            <Icon
              ...
              type="material-community"
              size={18}
              color={tabIndex === 2 ? '#193B6B' : '#8296B1'}
            />
          }
        />
        <Tab.Item
          ...
          icon={
            <Icon
              ...
              type="material-community"
              size={18}
              color={tabIndex === 3 ? '#193B6B' : '#8296B1'}
            />
          }
        />
      </Tab>
          ...
          >
            <View style={styles.sectionHeaderRow}>
              ...
              {canEditNutrition && nutritionPlan && (
                <TouchableOpacity
                  onPress={() =>
                    navigation.navigate('NutritionistTable', { patientId: patient?.patient_id })
                  }
              ...
              <View style={styles.nutritionCard}>
                <View style={styles.planHeader}>
                  <Text style={styles.planTitle}>
                    {nutritionPlan.title || t.staffPatientView.currentPlan}
                  </Text>
                ...
                <View style={styles.planBody}>
                  <View style={styles.dateInfoContainer}>
                    <View style={styles.dateSubBox}>
                      <Text style={styles.dateLabelText}>{t.staffPatientView.fromDate}</Text>
                    ...
                    />
                    <View style={styles.dateSubBox}>
                      <Text style={styles.dateLabelText}>{t.staffPatientView.toDate}</Text>
                      <Text style={styles.dateValueText}>
                  ...
                  <View style={styles.descriptionSection}>
                    ...
                    <Text style={styles.descContent}>
                      {nutritionPlan.description || t.staffPatientView.noDesc}
                    </Text>
                  </View>

                  <Divider style={{ marginVertical: 15 }} />

                  <Text style={[styles.sectionHeading, { fontSize: 16, marginBottom: 10 }]}>
                    {t.staffPatientView.dailyMeals}
                  </Text>
                  <MealItem
                    label={t.staffPatientView.breakfast}
                    content={nutritionPlan.breakfast}
                    icon="coffee-outline"
                    color="#A32D2F"
                  />
                  <MealItem
                    label={t.staffPatientView.lunch}
                    content={nutritionPlan.lunch}
                    icon="food-turkey"
                    color="#DE1A1C"
                  />
                  <MealItem
                    label={t.staffPatientView.dinner}
                  ...
                  <InfoItem
                    label={t.staffPatientView.allowedItems}
                    value={nutritionPlan.allowed_items || nutritionPlan.allowedItems}
                    icon="check-decagram"
                    color="#26CDD6"
                  />
                  <InfoItem
                    label={t.staffPatientView.forbiddenItems}
          ...
          >
            ...
            {sessions.length > 0 ? (
              sessions.map((session, index) => {
                const statusConfig = {
                  COMPLETED: {
                    label: t.patientProfile.status.completed,
                    bg: '#E9FAFB',
                    text: '#193B6B',
                  ...
                  IN_PROGRESS: {
                    label: t.patientProfile.status.pending,
                    bg: '#FBEAEA',
                    text: '#A32D2F',
                    icon: 'progress-clock',
                    borderColor: '#A32D2F',
                  ...
                  SCHEDULED: {
                    label: t.patientProfile.status.scheduled || 'مجدولة',
                    bg: '#E9FAFB',
                    text: '#193B6B',
                    icon: 'calendar-clock',
                    borderColor: '#26CDD6',
                  ...
                  >
                      ...
                      <View style={styles.sessionMetricBox}>
                        ...
                        <Text style={[styles.metricValue, { fontSize: 13, color: '#26CDD6' }]}>
                          {(() => {
                            ...
                            const lastSetting =
                              settingsArr.length > 0 ? settingsArr[settingsArr.length - 1] : null;
                            const ufRaw = parseFloat(
                              lastSetting?.ultrafiltration_rate ?? lastSetting?.ultrafiltrationRate
                            );
                    ...
                    <View style={styles.sessionTapHint}>
                      ...
                      <Text style={styles.sessionTapHintText}>
                        {t.staffPatientView.tapDetailsHint}
                      </Text>
                    </View>
                  </TouchableOpacity>
                );
              ...
              <View style={styles.emptyState}>
                ...
                <Text style={styles.emptyText}>{t.staffPatientView.noSessions}</Text>
              </View>
            )}
          </ScrollView>
        )}
            ...
            <View style={styles.subTabContainer}>
              ...
              >
                <Text style={[styles.subTabText, subTabIndex === 0 && styles.subTextActive]}>
                  {t.staffPatientView.subTabs.medications}
                </Text>
              </TouchableOpacity>
              <TouchableOpacity
              ...
              >
                <Text style={[styles.subTabText, subTabIndex === 1 && styles.subTextActive]}>
                  {t.staffPatientView.subTabs.lab}
                </Text>
              </TouchableOpacity>
              <TouchableOpacity
              ...
              >
                <Text style={[styles.subTabText, subTabIndex === 2 && styles.subTextActive]}>
                  {t.staffPatientView.subTabs.radiology}
                </Text>
              </TouchableOpacity>
            </View>
                ...
                <View>
                  ...
                  {prescriptions.length > 0 ? (
                          ...
                          <View style={{ alignItems: 'flex-end' }}>
                            ...
                            <View
                              style={[
                                ...
                                {
                                  backgroundColor:
                                    item.dispense_status === 'DISPENSED' ? '#E9FAFB' : '#FBEAEA',
                                },
                              ]}
                            >
                              <Text
                                style={[
                                  ...
                                  {
                                    color:
                                      item.dispense_status === 'DISPENSED' ? '#193B6B' : '#A32D2F',
                                  },
                                ]}
                              >
                                {item.dispense_status === 'DISPENSED'
                                  ? t.staffPatientView.dispensed
                                  : t.staffPatientView.pendingDispense}
                              </Text>
                            </View>
                          </View>
                          ...
                          >
                            ...
                            >
                              <View style={styles.drugNameRow}>
                                ...
                                >
                                  {drug.drug_name}{' '}
                                  {!drug.is_active && `(${t.staffPatientView.canceled})`}
                                </Text>
                              </View>
                              <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                <Text
                                  style={{
                                    fontSize: 11,
                                    color: drug.status ? '#26CDD6' : '#A32D2F',
                                    marginRight: 4,
                                  }}
                                >
                                  {drug.status
                                    ? t.staffPatientView.drugDispensed
                                    : t.staffPatientView.drugNotDispensed}
                                </Text>
                                <Icon
                                  name={drug.status ? 'check-circle' : 'clock-outline'}
                                  type="material-community"
                                  size={18}
                                  color={drug.status ? '#26CDD6' : '#A32D2F'}
                                />
                    ...
                    <View style={styles.emptyState}>
                      ...
                      <Text style={styles.emptyText}>{t.staffPatientView.noMedications}</Text>
                    </View>
                  )}
                </View>
              )}
                ...
                (medicalTests.length > 0 ? (
                  ...
                  <View style={styles.emptyState}>
                    ...
                    <Text style={styles.emptyText}>
                      {t.staffPatientView.noLabTests || 'لا توجد فحوصات مختبرية'}
                    </Text>
                  </View>
                ))}
              {subTabIndex === 2 &&
                (radiology.length > 0 ? (
                  ...
                  <View style={styles.emptyState}>
                    ...
                    />
                    <Text style={styles.emptyText}>
                      {t.staffPatientView.noRadiology || 'لا توجد صور أشعة'}
                    </Text>
                  </View>
                ))}
            </ScrollView>
        ...
        {tabIndex === 3 && (
          ...
          >
            <Text style={styles.sectionHeading}>
              {t.appointments?.completedTitle || 'سجل استشارات العيادة'}
            </Text>
            ...
            {consultations.filter((a) => a.status === 'COMPLETED').length > 0 ? (
                  ...
                  >
                    <View style={styles.consultHeader}>
                      <View style={styles.consultDocRow}>
                        ...
                        <View style={{ flex: 1 }}>
                          <Text style={styles.consultDocName}>
              ...
              <View style={styles.emptyState}>
                ...
                />
                <Text style={styles.emptyText}>
                  {t.appointments?.noCompleted || 'لا توجد استشارات مكتملة'}
                </Text>
              </View>
            )}
          </ScrollView>
