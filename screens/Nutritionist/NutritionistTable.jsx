  ...
  const fetchCurrentPlan = async () => {
    try {
      ...
      if (Array.isArray(data) && data.length > 0) {
        ...
        if (plan.startDate || plan.start_date)
          setStartDate(new Date(plan.startDate || plan.start_date));
  ...
  const handleSave = async () => {
    try {
      ...
      if (response.status === 200 || response.status === 201) {
        Alert.alert('✅ تم', 'تم حفظ البرنامج بنجاح', [
        ...
        <ScrollView style={styles.container} contentContainerStyle={{ paddingBottom: 40 }}>
          <View style={styles.headerContainer}>
            ...
            <Text style={styles.headerTitle}>
              {programId ? 'تعديل البرنامج الغذائي' : 'إنشاء برنامج جديد'}
            </Text>
            <View style={{ width: 40 }} />
          </View>
          ...
          <View style={styles.formCard}>
            ...
            <TextInput
              ...
              style={styles.input}
              placeholder="مثال: دايت الأسبوع 2"
            />
            ...
            <View style={styles.rowBetween}>
              <View style={{ width: '48%' }}>
                <Text style={[styles.label, { color: 'red' }]}>الممنوعات</Text>
                <TextInput
                  value={forbiddenItems}
                  onChangeText={setForbiddenItems}
                  style={[styles.input, { height: 80 }]}
                  multiline
                />
              </View>
              <View style={{ width: '48%' }}>
                <Text style={[styles.label, { color: 'green' }]}>المسموحات</Text>
                <TextInput
                  value={allowedItems}
                  onChangeText={setAllowedItems}
                  style={[styles.input, { height: 80 }]}
                  multiline
                />
              </View>
            ...
            <TouchableOpacity
              onPress={handleSave}
              style={[styles.saveBtn, { backgroundColor: programId ? '#26CDD6' : '#193B6B' }]}
            >
