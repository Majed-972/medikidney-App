...
const extractId = (obj) => obj?.id || obj?.settingId || obj?.setting_id || obj?.setting_id;
...
const SettingsTab = ({ route }) => {
  ...
  const FIELDS = [
    {
      key: 'bloodFlowRate',
      label: t.deviceSettings.fields.bloodFlow,
      placeholder: '200',
      unit: 'ml/min',
      icon: 'water-pump',
      color: '#DE1A1C',
    ...
    {
      key: 'dialysateFlow',
      label: t.deviceSettings.fields.dialysateFlow,
      placeholder: '500',
      unit: 'ml/min',
      icon: 'beaker-outline',
      color: '#26CDD6',
    ...
    },
    {
      ...
      label: 'السوائل المسحوبة',
      placeholder: 'مثال: 2.5',
      unit: 'لتر/مل',
      icon: 'water-percent',
      color: '#26CDD6',
      hint: 'Fluid Removed / Ultrafiltration',
    },
  ];
      ...
      if (recentSessions.length > 0) {
        ...
        recentSessions.forEach((s) => {
          ...
          if (isNaN(ufValue) || ufValue <= 0) {
            const settingsWithUF =
              s.dialysisSettings?.filter((set) => set.ultrafiltration_rate != null) || [];
            if (settingsWithUF.length > 0) {
              const rate = parseFloat(
                settingsWithUF[settingsWithUF.length - 1].ultrafiltration_rate
              );
  ...
  const handleDelete = () => {
    ...
    Alert.alert(t.deviceSettings.deleteTitle, t.deviceSettings.deleteMsg, [
      ...
      {
        text: t.deviceSettings.deleteBtn,
        style: 'destructive',
        onPress: async () => {
          try {
            setLoading(true);
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <View style={styles.formCard}>
        <Text style={styles.formTitle}>
          {saved ? t.deviceSettings.update : t.deviceSettings.addNew}
        </Text>
        {FIELDS.map((f) => {
                  ...
                  <View style={{ flexDirection: 'row-reverse', gap: 10 }}>
                    ...
                    >
                      <View style={{ alignItems: 'center' }}>
                        ...
                        <View
                          style={{ flexDirection: 'row-reverse', alignItems: 'flex-end', gap: 4 }}
                        >
                          <Text
                            style={[
                              styles.input,
                              { color: '#193B6B', flex: 0, textAlign: 'center' },
                            ]}
                          >
                            {prevWeightAfter}
                          </Text>
                      ...
                      <View style={{ alignItems: 'center' }}>
                        ...
                        >
                          <Text
                            style={[
                              styles.input,
                              { color: '#26CDD6', flex: 0, textAlign: 'center' },
                            ]}
                          >
                            {currWeightBefore}
                          </Text>
                ...
                {monthlyAverageUF !== null && (
                  <View style={styles.monthlyAvgBox}>
                    <MaterialCommunityIcons
                      ...
                      color="#26CDD6"
                    />
                    <Text style={styles.monthlyAvgText}>
                      معدل السحب لآخر شهر: {monthlyAverageUF} لتر
                    </Text>
                  </View>
                )}
              </View>
          ...
          {isSubmitting ? (
            ...
            <>
              ...
              <Text style={styles.saveBtnText}>
                {saved ? t.deviceSettings.update : t.deviceSettings.save}
              </Text>
            </>
          )}
        </Pressable>
