...
const SessionTimer = ({ session, size = 'small' }) => {
  ...
  const shouldAnimate = ready && remaining > 0;
  ...
  return (
    <View style={smallStyles.container}>
      <View style={smallStyles.row}>
        ...
        <Text style={smallStyles.label}>{remaining > 0 ? 'الوقت المتبقي' : 'انتهى الوقت'}</Text>
      </View>
      <View style={smallStyles.barBg}>
        <View
          style={[smallStyles.barFill, { width: `${progress * 100}%`, backgroundColor: barColor }]}
        />
      </View>
    </View>
  );
