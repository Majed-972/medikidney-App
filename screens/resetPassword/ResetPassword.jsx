...
const ResetPassword = ({ route, navigation }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await ValidationPassword.validate({ newPassword, confirmPassword }, { abortEarly: false });
      ...
      if (err.inner) {
        ...
        Alert.alert('خطأ من السيرفر', err.response?.data?.message || 'فشل الاتصال بالسيرفر');
      } else {
...
const styles = StyleSheet.create({
  container: { flex: 1, justifyContent: 'center', padding: 16, backgroundColor: '#F1FCFD' },
  header: {
    fontSize: 22,
    fontWeight: 'bold',
    color: '#26CDD6',
